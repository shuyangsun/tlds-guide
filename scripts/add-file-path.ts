...
/** Regex patterns for directories to ignore (format: /^.*\/dirname$/) */
const IGNORE_DIR_PATTERNS: RegExp[] = [
  /^\.git$/,
  /^\.vscode$/,
  /^\.husky$/,
  /^.*\.wrangler$/,
...
/** Regex patterns for files to ignore (format: /^.*\/filename$/) */
...
function shouldIgnoreDirectory(dirPath: string): boolean {
  return IGNORE_DIR_PATTERNS.some((pattern) => pattern.test(dirPath));
}
...
function getFilesToProcess(inputPath: string, projectRoot: string): string[] {
  ...
  try {
    ...
    if (stat.isFile()) {
      // Check if the file matches any supported extension and is not ignored
      const relativePath = relative(projectRoot, absolutePath);
      if (getHeaderConfig(absolutePath) && !shouldIgnoreFile(relativePath)) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const relPath = relative(projectRoot, fullPath);
        if (config && !shouldIgnoreFile(relPath)) {
          files.push(fullPath);
        }
      }
    }
